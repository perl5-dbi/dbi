...
sub run_tests {
    ...
    my $test_run_tag =
	"Testing $transport transport with $policy_name policy @{[ %$connect_attr ]}";
    print "\n$test_run_tag\n";
	...
	$driver_dsn .= join ";", "",
	    map { "$_=$trans_attr->{$_}" } keys %$trans_attr
	    if %$trans_attr;
    ...
    my $dbh =
	$DBI->connect ($dsn, undef, undef, {%$connect_attr, RaiseError => 1});
    ...
    $start = time ();
    $ins_sth->execute ($_, "speed") for (1000 .. 1000 + $opt_count);
